...
inst.query('*opc?')                         # wait until scope has finished acquiring waveforms
...
yoffset = float(inst.query('wfmoutpre:yoff?'))
...
numberofpoints = int(inst.query('wfmoutpre:nr_pt?'))
# amount of time between data points
xincrement = float(inst.query('wfmoutpre:xincr?'))
