...
for playback.
The user MUST input the path of the text file and input the number
of desired bits in the waveform. Currently the script requires that the 
number of bits in the waveform is an integer multiple of the number 
of bits in the data file.
Windows 7 64-bit
...
centerFreq = 1e9
numSym = 128
fileName = 'C:\\Users\\mallison\\documents\\Matlab\\testpattern2.txt'
# ###############################INPUT USER PARAMETERS HERE
...
wlength = int(awg.ask('wlist:waveform:length? {}'.format(wname.rstrip())))
# print(wlength)
...
for i in range(repeats):
    for j in range(numBits):
        # the AWG only uses the 2 most significant bits in a byte to set
        # individual marker1 and marker2 samples
        if markerValues[j] == 0:
            markerData[start:stop] = int('10000000', 2)
        elif markerValues[j] == 1:
            markerData[start:stop] = int('01000000', 2)
        start = stop
        stop = start + sampPerSym
"""
one = np.empty(sampPerSym, dtype=np.uint8)
one.fill(64)
zero = np.empty(sampPerSym, dtype=np.uint8)
zero.fill(128)
markerData = []
for i in range(repeats):
	for j in range(numBits):
		if markerValues[j] == 0:
			markerData = np.append(markerData, zero)
		elif markerValues[j] == 1:
			markerData = np.append(markerData, one)

# Convert marker data to 8-bit format
# markerData = list(markerData)
stringArg = 'wlist:waveform:marker:data {}, 0, {}, '.format(wname, wlength)
awg.write_binary_values(stringArg, markerData)
